...
#source(paste(FRK_WORKING_DIR, 'frk_utils.R', sep='/'))
...
## Load data "data_air"
data_air <- readRDS(paste0(Current_Dir, '/data_air.rds'))
...
# ==============================================================================
### Covariate
comuni <- readRDS(paste0(Current_Dir, '/comuni.rds'))
library(terra)
#data_elevazione <- readRDS(paste0(Current_Dir, '/data_elevazione.rds')) %>% rast
data_elevazione <- rast(paste0(Current_Dir, '/data_elevazione.tif'))

data_popolazione_smoothed_nodes <- readRDS(paste0(Current_Dir, '/data_popolazione_smoothed_nodes.rds'))
...
colnames.euclid = sapply(1:(length(data_air$levels) - 1), FUN=function(i){paste0("Y", as.character(i))})

IndexDrop = 0            # (no drop)
flag_use_pca = FALSE     # pca
...
#GridBAUs1 <- GetGridBAUs(data_air$grid, cellsize=baus.cellsize)
...
GridBAUs <- DoBaus_WithCov(GridBAUs1, cov.P1 = cov.P1.trans, cov.Elev = cov.Elev.trans)
...
result <- CFRK (data.comp = data_air$dataset, flag_use_pca, flag_bau_comunali, flag_obs_fs, baus.cellsize, frk.args,
       ...
       Use_Global_Colors = Use_Global_Colors, Colors_Breaks = Colors_Breaks, DoPLOT = TRUE, FLAG_NO_PLOT = FLAG_NO_PLOT)
...
# plot
comuniresult <- CFRKPlotPrediction (data.comp = data_air$dataset, domain = data_air$domain,
...
comuniresult.2 <- cbind(comuniresult,
  as.data.frame(res.confintervals$prediction.comuni)[, c("Y.pred.upper","Y.pred.lower","Y.pred.upper.95","Y.pred.lower.95","Y.pred.upper.99","Y.pred.lower.99")])

# saveRDS(result, file=paste0(OUTPUT_MAIN_DIR, "/result_cfrk.rds"))

# save
CFRKSave (comuniresult.2, exporting_colnames = c(colnames.aitch.pred,"Y.pred.upper","Y.pred.lower","Y.pred.upper.95","Y.pred.lower.95","Y.pred.upper.99","Y.pred.lower.99"), folder_output,
